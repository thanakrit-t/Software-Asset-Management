...
begin
  ...
  loop
    ...
    insert into public.license_entitlements (
      ...
      license_row.normalized_start_date,
      case when license_row.normalized_start_date is not null and license_row.normalized_end_date is not null
        and license_row.normalized_start_date > license_row.normalized_end_date
        then null else license_row.normalized_end_date end,
      ...
      case when license_row.normalized_start_date is not null and license_row.normalized_end_date is not null
        and license_row.normalized_start_date > license_row.normalized_end_date
        then pg_catalog.concat_ws(' | ',nullif(license_row.raw_data->>'remark',''),
          'Source end date precedes start date; original retained in migration staging')
        else nullif(license_row.raw_data->>'remark','') end,
